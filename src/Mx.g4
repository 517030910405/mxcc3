...
stat : singstat* ;
singstat : expr';'#sing_stmt|type_and_var';'#assign_stmt| Return__ expr*';'#return_stmt
...
expr : This__#exprthis|int_#id1|bool_#id1|var_#id2|string_#id1|null__#id1|void__#id1|'(' expr ')'#exprmix|expr op_in_expr=('++'|'--')#opl
    |expr '.' expr #exprsub|expr'.''substring''('expr','expr')'#substringfunc
    |expr '.''parseInt''('')'#parseintfunc|expr'.''ord''('expr')'#ordfunc|expr'.''size' '('  ')'#sizefunc|expr'.''length' '('  ')'#lengthfunc|expr '[' expr? ']'#exprarr|expr '(' (expr(',' expr)*)? ')'#exprfunc
